...
void
DiscreteFourierTransform80(int n)
{
	...
	*/
	...
	fprintf(stats[TEST_FFT80], "\t\tCOMPUTATIONAL INFORMATION:\n");
	fprintf(stats[TEST_FFT80], "\t\t-------------------------------------------\n");
	fprintf(stats[TEST_FFT80], "\t\t(a) Percentile = %f\n", percentile);
	fprintf(stats[TEST_FFT80], "\t\t(b) N_l        = %f\n", N_l);
	fprintf(stats[TEST_FFT80], "\t\t(c) N_o        = %f\n", N_o);
	fprintf(stats[TEST_FFT80], "\t\t(d) d          = %f\n", d);
	fprintf(stats[TEST_FFT80], "\t\t-------------------------------------------\n");
