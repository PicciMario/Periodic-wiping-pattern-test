...
main(int argc, char *argv[])
{
	...
	if ( argc < 2 ) {
		printf("\nUsage: %s <stream length> <data file> <bit streams>\n", argv[0]);
		printf("   <stream length> is the length of the individual bit stream(s) to be processed\n");
		printf("   <data file>     is the input file to read the streams from\n");
		printf("   <bit streams>   is the number of individual bit stream(s) to be processed\n");
		printf("   <testing log>   is the file to append the results of the testing to\n");
		printf("\n");
	...
	if (argc >= 3)
		strcpy(tp.inputFileName, argv[2]);
	
	if (argc >= 4)
		tp.numOfBitStreams = atoi(argv[3]);
	else
		tp.numOfBitStreams = 0;

	if (argc >= 5)
		strcpy(tp.outputFileName, argv[4]);
